...
# reason_output 9 sınıfın tümünü kapsar (ağlama nedenleri + ağlama olmayan sesler).
# top_reason yalnızca gerçek bir "ağlama nedeni" olduğunda anlamlıdır; bu yüzden
# ağlama nedeni sınıflarını, ağlama olmayan sınıflardan ayırt ediyoruz.
...
def ses_analiz_et(file_path):
    ...
    # top_reason yalnızca ağlama tespit edildiğinde ve yalnızca gerçek ağlama nedeni
    # sınıfları (hungry, bellypain, ...) arasından seçilir. Aksi halde, bebek
    # ağlamıyorken bile "laugh"/"silence"/"noise" gibi sınıflar "ağlama nedeni"
    # olarak raporlanabilir, bu da yanıltıcı olur.
    top_reason = None
    if is_crying:
        cry_reason_probs = [p for p in probabilities if p["label"] in CRY_CLASSES]
        if cry_reason_probs:
            top_reason = cry_reason_probs[0]["label"]
