...
# (app.py ile aynı şekilde, MODEL_YOLU / SINIF_YOLU ortam değişkenleriyle
# özelleştirilebilir; belirtilmezse proje kökündeki dosyalar kullanılır.)
...
def ses_analiz_et(file_path):
    ...
    # reason_output, ağlama nedenlerinin yanı sıra laugh/silence/noise gibi
    # ağlama olmayan sınıfları da içerir. Bu yüzden en yüksek ihtimalli sınıfı
    # yalnızca ağlama tespit edildiğinde "ağlama nedeni" olarak sunuyoruz;
    # aksi halde yanıltıcı olur (örn. ağlama yokken "En olası neden: Tired" demek gibi).
    if is_crying:
        cry_classes = {"hungry", "bellypain", "tired", "cold_hot", "discomfort", "burping"}
        if en_olasi_neden in cry_classes:
            print(f"EN OLASI AĞLAMA NEDENİ: {str(en_olasi_neden).capitalize()}")
        else:
            print("EN OLASI AĞLAMA NEDENİ: Belirlenemedi (model ağlama nedeni sınıflarından birini baskın bulamadı).")
    else:
        print("NOT: Ağlama tespit edilmediği için bir 'ağlama nedeni' raporlanmıyor.")
    print("=======================================================")
